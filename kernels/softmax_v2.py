# source from https://github.com/FlagOpen/FlagGems.git
...
from kernels.utils import triton_lang_extension as tle
from kernels.utils.configs import softmax_tuned_configs, softmax_heuristic_configs

"""
对于任意形状的张量进行softmax, 按照reduce轴的位置可以分成两类:
[pre_dim, reduce_dim, post_dim]
1. post_dim  = 1 (inner), 相当于对每一行进行softmax
    num_program如果大于sm的数量, 则可以通过persistent的方式来完成(一个block计算多行, step=num_program)
    if reduce_dim较小(< 32K)
        block直接将完整的一行全部加载到寄存器中此时仅需要一次读一次写
    else
        block每次处理TILE_N个元素, 然后通过online_softmax的方式, 通过两次读一次写完成运算
        1. 当TILE_N<N时, 对于前面没有超出N的时候不需要mask, 特化处理第一次读的最后一轮(mask)
        2. 第二次读并写回时采用reverse traverse增加L2缓存命中率
        3. 第二次读并写回时指定缓存策略tl.load(eviction_policy="evict_first")
2. post_dim != 1 (non_inner), 相当于对每一列进行softmax
    随着post_dim增大, TILE_K也应该相应增大(一般在16以内)
    if reduce_dim较小
        一次性将TILE_K列全部读入和inner的情况类似
    else
        此时特化处理的收益不大, 仅使用后两种trick即可

"""

@triton.heuristics(softmax_heuristic_configs("softmax_non_inner"))
@triton.jit
def softmax_kernel_non_inner(
    ...
    if ONE_TILE_PER_CTA:
        ...
        # specialization does not improve performance in this example, as tested
...
@triton.heuristics(softmax_heuristic_configs("softmax_inner"))
@triton.jit
def softmax_kernel_inner(
    ...
    pid_m = tle.program_id(0)
    if ONE_TILE_PER_CTA:
        # 如果一个block处理的一行元素被一次性载入并处理，也就是矩阵的列数小于等于32k的时候
        n_offsets = tl.arange(0, TILE_N)
        ...
        z = tl.full([TILE_N], value=0.0, dtype=tl.float32)  # z = sum e^(x-m)
        input_ptr += pid_m * N
        output_ptr += pid_m * N
        ...
        for start_n in range(previous_multiple, N, TILE_N):  # why its a loop?
            n_offsets = start_n + tl.arange(0, TILE_N)
            mask = n_offsets < N
...
# ------------------------  backward -------------------------------
@triton.autotune(
    configs=softmax_tuned_configs("softmax_non_inner"),
    key=[
        "M",
        "N",
        "K",
...
@triton.heuristics(softmax_heuristic_configs("softmax_backward_non_inner"))
@triton.jit
...
@triton.autotune(
    configs=softmax_tuned_configs("softmax_inner"),
    key=["M", "N"],
)
@triton.heuristics(
    values=softmax_heuristic_configs("softmax_backward_inner"),
)
@triton.jit
